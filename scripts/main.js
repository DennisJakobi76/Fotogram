...
let images = ["angel_of_the_north.avif", "burgruine_hardenstein.avif", "burg_blankenstein.avif", "celtic_cross.avif", "irish_landsight.avif", "netherlands_evening.avif", "nevermore.avif", "roses_of_westerholt.avif", "sun_over_sea.avif", "the_long_hall.avif", "view_to_an_angel.avif", "westerholt_leuchtet.avif"];

function renderImages() {
    for (i = 0; i < images.length; i++) {
        content_container.innerHTML += getImgHtml(path, images[i], `Picture of ${images[i].substring(0, images[i].length - 4)}`, i);
    }
}
...
function getNextImageLeft() {
    ...
    overlay_navCounter.innerText = renderNewCounter(imgCounterDown + 1, allImages.length);
...
function getNextImageRight() {
    ...
    overlay_navCounter.innerText = renderNewCounter(imgCounterUp + 1, allImages.length);
